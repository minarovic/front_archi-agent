...
**→ [Tool 0 Detailed Documentation](./tool0-detailed-flow.md)**

Pro detailní informace o Tool 0, včetně:
- ✅ Kompletní architektura diagram
- ✅ Input Markdown template
- ✅ Output Pydantic schema
- ✅ Implementation pattern (Direct OpenAI SDK + JSON mode)
- ✅ Azure configuration
- ✅ Parsing logic s error handling
- ✅ Sample files & testing
- ✅ Known limitations & future enhancements

**Klíčové vlastnosti:**
- 🟠 **Pattern A:** Direct OpenAI SDK s Azure endpoint (ne LangChain)
- 📝 **JSON mode:** Garantované strukturované výstupy
- ✅ **Pydantic validation:** Post-LLM schema kontrola
- 🌍 **Czech/English:** Multilingvální podpora
- ⚡ **Timing:** ~3-5s průměr (target <10s ✅)
...
Pro detailní vizualizaci 5-node architektury Tool 1 včetně všech interních kroků,
state transformací a timing informací, viz:

**→ [Tool 1 Detailed Flow Diagram](./tool1-detailed-flow.md)**

Diagram zahrnuje:
- ✅ Kompletní flow všech 5 nodes (Load, Prepare LLM, Mapping LLM, Filter, Save)
- ✅ State transformace (Tool1State evolution)
- ✅ Timing breakdown (~23s total, target <40s ✅)
- ✅ Color-coded legenda (zelená=deterministic, oranžová=LLM, žlutá=output)
...
Pro detailní vizualizaci 5-node architektury Tool 2 včetně všech interních kroků,
hybridního přístupu (heuristics + LLM), classification decision tree a timing informací, viz:

**→ [Tool 2 Detailed Flow Diagram](./tool2-detailed-flow.md)**

Diagram zahrnuje:
- ✅ Kompletní flow všech 5 nodes (Load, Classify LLM, Relationships, Assemble, Save)
- ✅ Classification Decision Tree (prefix routing logic: factv_* vs dimv_*)
- ✅ State transformace (Tool2State evolution)
- ✅ Timing breakdown (~15s total, target <20s ✅)
- ✅ Color-coded legenda (zelená=deterministic, oranžová=LLM, béžová=hybrid, žlutá=output)
- ✅ Key Features: Hybrid approach, confidence scoring, FK detection, coverage tracking
